...
prompt * Grant access rights, if necessary
declare
  cursor grant_cur is
    with users as (
           select username grant_user
             from all_users
            where username in (upper('&LC_APEX.'), upper('&LC_REST.')))
    select object_name, object_type, grant_user
      from all_objects
     cross join users
     where object_type in ('TABLE', 'VIEW', 'PACKAGE')
       and object_name in (
             'HR_EMPLOYEES', 'HR_DEPARTMENTS', 'HR_JOBS', 'HR_LOCATIONS', 'HR_COUNTRIES', 'HR_REGIONS', 'HR_EMP_DETAILS',
             'BL_EMP', 'BL_DEP')
     order by grant_user, object_type, object_name;
begin
  for obj in grant_cur loop
    if obj.object_type in ('TABLE', 'VIEW') then
      execute immediate 'grant read on '  || obj.object_name || ' to ' || obj.grant_user || ' with grant option';
      dbms_output.put_line('.  read privilege on ' || obj.object_name || ' granted to ' || obj.grant_user);
    else
      execute immediate 'grant execute on '  || obj.object_name || ' to ' || obj.grant_user;
      dbms_output.put_line('.  execute privilege on ' || obj.object_name || ' granted to ' || obj.grant_user);
    end if;
  end loop;
end;
/